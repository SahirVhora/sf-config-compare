...
def _run_migrations(conn: sqlite3.Connection) -> None:
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_pull_history_instance ON pull_history(instance_id, pull_type)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_pull_history_started ON pull_history(started_at)")
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_entity_snapshots_history ON entity_snapshots(history_id)")
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_picklist_snapshots_history ON picklist_snapshots(history_id)")
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_drift_results_check ON drift_results(check_id)")
        ...
        if history_id:
            conn.execute(
                """UPDATE pull_history
                   ...
                   WHERE id = ?""",
                (status, entities_count, fields_count, picklists_count, values_count, error, history_id),
            )
            return history_id
        cur = conn.execute(
...
def get_pull_history(instance_id: int | None = None, pull_type: str | None = None, limit: int = 100):
...
def get_pull_history_by_id(history_id: int) -> dict | None:
    with get_conn() as conn:
        row = conn.execute("SELECT * FROM pull_history WHERE id = ?", (history_id,)).fetchone()
        return dict(row) if row else None
        ...
        for ent in entities:
            conn.execute(
                """INSERT INTO entity_snapshots
                   ...
                   VALUES (?, ?, ?, ?, ?)""",
                (history_id, ent["entity_name"], ent.get("entity_label", ""),
                 ent.get("element_name", ""), json.dumps(ent.get("fields", []))),
            )
        ...
        for pv in picklist_values:
            conn.execute(
                """INSERT INTO picklist_snapshots
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?)""",
                (history_id, pv["picklist_id"], pv.get("external_code"), pv.get("option_id"),
                 pv.get("label_en"), pv.get("status"), pv.get("all_labels")),
            )
    ...
    with get_conn() as conn:
        cur = conn.execute(
            """INSERT INTO scheduled_checks
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (data["name"], data["instance_a_id"], data["instance_b_id"],
             data.get("cron_expression", "0 0 * * *"), int(data.get("enabled", True)),
             data.get("webhook_url"), data.get("webhook_type", "slack"),
...
def get_scheduled_check(check_id: int) -> dict | None:
    with get_conn() as conn:
        row = conn.execute("SELECT * FROM scheduled_checks WHERE id = ?", (check_id,)).fetchone()
        return dict(row) if row else None
        ...
        conn.execute(
            """UPDATE scheduled_checks
               SET name = ?, cron_expression = ?, enabled = ?,
                   webhook_url = ?, webhook_type = ?, notify_on = ?
               WHERE id = ?""",
            (data["name"], data.get("cron_expression", "0 0 * * *"),
             int(data.get("enabled", True)), data.get("webhook_url"),
             data.get("webhook_type", "slack"), data.get("notify_on", "any_change"),
             check_id),
        )
...
def record_drift_result(check_id: int, status: str, summary_json: str | None = None,
                        entity_diff_count: int = 1, field_diff_count: int = 1,
                        picklist_issue_count: int = 1, report_id: str | None = None) -> int:
    with get_conn() as conn:
        cur = conn.execute(
